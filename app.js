...
document.getElementById("scoring").onclick = async function () {
  ...
  // htmlに順位、名前、点数を表示
  console.log(scores);
  var result_array = Object.keys(scores).map((k) => ({
    key: k,
    value: scores[k],
  }));
  result_array.sort((a, b) => a.value - b.value);
  ...
  document
    ...
    .insertAdjacentHTML("beforeend", `<h2>結果</h2>`);
    rank = 1;
    before_point = -1;
  for (let i = 0; i < result_array.length; i++) {
    if(before_point != result_array[i]["value"]){
      rank = i+1
    }
  
    var result = `<p>${rank}位 ${result_array[i]["key"]}さん ${
      result_array[i]["value"]
    }点</p>`;
    document.getElementById("result").insertAdjacentHTML("beforeend", result);
    before_point = result_array[i]["value"]
  }
}